...
$stmt = $pdo->prepare("SELECT * FROM password_resets_client WHERE token = ? AND expires_at > NOW()");
$stmt->execute([$token]);
$reset = $stmt->fetch();
...
$stmtUser = $pdo->prepare("SELECT * FROM client WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($password) || empty($confirmPassword)) {
        ...
        $stmtUpdate = $pdo->prepare("UPDATE client SET password = ? WHERE id = ?");
        $stmtUpdate->execute([$hashedPassword, $user['id']]);

        // Supprimer le token pour éviter toute réutilisation
        $stmtDel = $pdo->prepare("DELETE FROM password_resets_client WHERE token = ?");
            ...
            <div class="col-lg-8">
                <div class="reset-container">
                    <div class="reset-header">
                        ...
                        <p class="mb-0">Créez un nouveau mot de passe sécurisé pour votre compte client</p>
                    </div>
                    ...
                    <div class="reset-body">
                        ...
                        <?php if(isset($success)): ?>
                            ...
                            <div class="text-center mt-4">
                                <a href="connexion_client.php" class="btn btn-primary btn-lg">
